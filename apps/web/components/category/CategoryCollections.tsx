  ...
  return (
    <div className="flex flex-col flex-1">
      <div className="flex items-center justify-between mb-6">
        <h3 className="text-xl font-bold font-outfit text-white">Popular Puzzle Collections</h3>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {collections.map((collection, index) => (
          <Link key={index} href="#" className={`flex flex-col items-center justify-center p-6 rounded-2xl border bg-transparent hover:bg-white/5 transition-colors group cursor-pointer ${collection.color}`}>
            ...
            <h4 className="text-sm font-bold text-gray-300 text-center mb-1 group-hover:text-white transition-colors">
              {collection.title}
            </h4>
            <span className="text-xs text-gray-500 font-medium">{collection.games} Games</span>
          </Link>
        ))}
      </div>
    </div>
