...
export default function LatestArticles({ posts }: { posts: any[] }) {
  ...
  const allArticles = posts.slice(3);

  const filteredArticles = activeTab === 'All'
    ? allArticles
    : allArticles.filter((art) => {
        const titleMatch = art.title?.toLowerCase().includes(activeTab.toLowerCase());
        const tagMatch = art.tags?.some((t: string) => t.toLowerCase() === activeTab.toLowerCase());
      ...
      <div className="flex flex-col gap-4">
        {filteredArticles.length > 0 ? filteredArticles.map((article) => (
          <Link href={`/blog/${article.slug}`} key={article.slug} className="flex flex-col sm:flex-row gap-4 sm:gap-6 bg-white dark:bg-[#111228] border border-gray-200 dark:border-white/5 p-4 rounded-2xl hover:border-gray-300 dark:hover:border-white/10 shadow-sm dark:shadow-none transition-colors group">
            ...
            <div className="flex flex-col justify-center flex-1 py-1">
              ...
              <div className="flex items-center justify-between mt-auto">
                ...
                <span className="px-4 py-1.5 bg-[#6366F1]/10 text-[#6366F1] group-hover:bg-[#6366F1] group-hover:text-white text-xs font-bold rounded-lg transition-colors">
                  Read More
                </span>
              </div>
            </div>
          ...
          <div className="py-12 text-center text-gray-500">
            {activeTab === 'All' ? 'More articles coming soon!' : `No articles found in ${activeTab}.`}
          </div>
        )}
      </div>
