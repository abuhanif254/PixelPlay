...
export default function ArticleContent() {
  ...
  return (
    <article className="prose dark:prose-invert max-w-none prose-headings:font-outfit prose-headings:font-bold prose-p:text-gray-700 dark:prose-p:text-gray-300 prose-p:leading-relaxed prose-a:text-[#6366F1] hover:prose-a:text-[#5457DF] mb-12">
      
      <h2 id="section-1" className="text-2xl md:text-3xl text-gray-900 dark:text-white mt-0 mb-4">1. What Makes a Great Adventure Game?</h2>
      ...
      <h2 id="section-2" className="text-2xl md:text-3xl text-gray-900 dark:text-white mb-6">2. Our Top 10 Adventure Games for 2024</h2>
          ...
          <div key={game.id} className="flex flex-col sm:flex-row gap-6 p-1 bg-transparent">
            ...
            <div className="flex flex-col justify-center flex-1 py-1">
              ...
              <div className="flex items-center gap-1 mb-3">
                {[1, 2, 3, 4, 5].map((star) => (
                  <svg key={star} className={`w-4 h-4 ${star <= 4 ? 'text-[#6366F1]' : 'text-gray-300 dark:text-gray-600'}`} fill="currentColor" viewBox="0 0 20 20">
                    <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
                  </svg>
                ))}
                <span className="text-sm font-bold text-gray-700 dark:text-gray-300 ml-2">{game.rating}</span>
              </div>
              ...
              <div className="flex flex-wrap gap-2 mt-auto">
                {game.tags.map((tag, idx) => (
                  <span key={idx} className="px-3 py-1 rounded-full border border-gray-200 dark:border-white/10 text-gray-500 dark:text-gray-400 text-[11px] font-bold uppercase tracking-wider bg-transparent">
                    {tag}
                  </span>
                ))}
              </div>
