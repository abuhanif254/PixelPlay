...
export default function TopChampionCard() {
  return (
    <div className="bg-[#111228] border border-white/5 rounded-2xl p-6 shadow-xl mb-6 relative overflow-hidden flex flex-col items-center text-center">
      
      {/* Background Glow */}
      <div className="absolute top-0 left-1/2 -translate-x-1/2 w-48 h-48 bg-[#6366F1]/20 blur-[60px] rounded-full pointer-events-none" />

      <div className="flex items-center gap-2 text-white font-bold mb-6 z-10">
      ...
      <div className="relative mb-4 z-10">
        <svg className="absolute -top-10 left-1/2 -translate-x-1/2 w-14 h-14 text-yellow-400 drop-shadow-[0_0_15px_rgba(250,204,21,0.6)] z-20" fill="currentColor" viewBox="0 0 24 24">
        ...
        <div className="w-24 h-24 rounded-full border-4 border-yellow-400 overflow-hidden bg-white/10 p-1 relative z-10 shadow-[0_0_30px_rgba(250,204,21,0.3)]">
          <img src="https://api.dicebear.com/7.x/bottts/svg?seed=AlexGamer&backgroundColor=f59e0b" alt="Champion" className="w-full h-full rounded-full object-cover" />
        </div>
      </div>

      <h3 className="font-bold text-xl text-white mb-1 z-10">AlexGamer</h3>
      ...
      <Link href="#" className="w-full py-3 bg-[#6366F1] hover:bg-[#5457DF] text-white text-sm font-bold rounded-xl transition-all shadow-lg z-10">
        View Profile
      </Link>
    </div>
  );
